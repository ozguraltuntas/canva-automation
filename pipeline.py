...
def photoroom_edit(input_path: Path, api_key: str = None,
                   padding: float = 0.05) -> Image.Image:
...
def composite_on_template(car_rgba: Image.Image, template_path: Path,
    ...
    target_w = int(T_W * width_ratio)
    scale = target_w / car_rgba.width
    target_h = int(car_rgba.height * scale)
    car_r = car_rgba.resize((target_w, target_h), Image.LANCZOS)
    ...
    if add_shadow:
        ...
        canvas.paste(shadow_r, (x, y), shadow_r)
    
    canvas.paste(car_r, (x, y), car_r)
    return canvas
